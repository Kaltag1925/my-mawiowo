    ...
    public void render(Graphics g, int tick, float alpha)
    {
        ...
        for (int x = xCam / 16; x <= (xCam + width) / 16; x++)
            for (int y = yCam / 16; y <= (yCam + height) / 16; y++)
            {
                ...
                boolean valInIf =((Level.TILE_BEHAVIORS[b & 0xff]) & Level.BIT_ANIMATED) > 0;
                System.out.println(""); // apparently calling println here is neccessary
                if (valInIf)//((Level.TILE_BEHAVIORS[b & 0xff]) & Level.BIT_ANIMATED) > 0)
                {
